...
public class SDNDraw : MonoBehaviour {
	...
	void Start () {
		...
		targetNetwork.onNewNetwork += updateVisualNetworkHandler;
		createVisualNetwork ();
	...
	private void createVisualNetwork() {

		foreach (Transform child in drawParent.transform) {
			GameObject.Destroy(child.gameObject);
		}

		for (int i = 0; i < targetNetwork.targetNumReflections + 1; i++) {
	...
	private void updateVisualNetworkHandler(List<SDN.SDNnode> theNetwork, reflectionPath theDirectsound) {
		StartCoroutine(updateVisualNetwork(theNetwork,theDirectsound));
	}

	private IEnumerator updateVisualNetwork (List<SDN.SDNnode> theNetwork, reflectionPath theDirectsound)
	{

		List<SDN.SDNnode> network = theNetwork;
		reflectionPath directSound = theDirectsound;
		...
		foreach (SDN.SDNnode n in network) {

			reflectionPath p = n.getPath ();
