...
public class MeshOcclusionUIController : MonoBehaviour, ITangoLifecycle, ITangoPose
{
	...
	public GameObject m_textInstructions;
	...
	public GameObject Button_StartObjectfinding;
...
/// </summary>
	...
	public GameObject m_ToggleEnableTapToPlace;
	public InputField m_TestingVersionInput;
	...
	public bool enableTapToPlace = false;
	private bool startObjectFinding = false;
...
//		AudioSettings.Reset (AC);
		...
		m_numPathInput.text = SDN.targetNumReflections.ToString();
		m_specCoeffInput.text = SDN.SDNnode.specularity.ToString();
		m_nodeLossInput.text = SDN.SDNnode.nodeLoss.ToString();
		m_wallAbsInput.text = SDN.SDNnode.wallAbsCoeff.ToString();

		listenerTransform = audioRec.gameObject.transform;
	...
	public void beginObjectFinding() {
		...
		m_markerObject.GetComponent<AudioSource> ().Play ();
	...
	public void setTestingType() {
		...
		string input = m_TestingVersionInput.text;

		switch (input) {
		case "A":
			scatteringNetwork.enableListen = false;
			...
			break;
		case "B":
			scatteringNetwork.enableListen = true;
		...
		if (enableTapToPlace) {
			...
			if (Physics.Raycast (Camera.main.ScreenPointToRay (pdata.position), out hit, Mathf.Infinity)) {
				if (hit.collider.name != "boundary") {
					m_markerObject.SetActive (true);
//					m_markerObject.transform.position = hit.point + (hit.normal * 0.1f);
...
//		Vector3 randomDirection = UnityEngine.Random.insideUnitSphere * 10.0f;
//		randomDirection += transform.position;
//		NavMeshHit hit;
//		NavMesh.SamplePosition(randomDirection, out hit, 10.0f, 1);
//		m_markerObject.transform.position = hit.position;
