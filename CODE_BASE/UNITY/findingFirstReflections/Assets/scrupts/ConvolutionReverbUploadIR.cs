    ...
    */
    ...
    void UploadChangedClips()
    {
		if (!gameObject.GetComponent<generateWGWIR> ().doUpload ()) {
			...
			IR.GetData (data, 0);
			Debug.Log ("Uploading impulse response " + IR.name + " to slot 5 with data length: " + data.Length);
			ConvolutionReverb_UploadSample (4, data, data.Length / IR.channels, IR.channels, IR.frequency, IR.name);
		}
			/*
