...
public class WGWDraw : MonoBehaviour {
	...
	private WGW targetNetwork;
	private GameObject drawParent;

	void Start () {
		targetNetwork = gameObject.GetComponent<WGW> ();
	...
	public void createVisualNetwork() {
		...
		for (int i = 0; i < WGW.targetNumReflections + 1; i++) {
			...
			if (i > 0) {
				...
				junction.transform.localScale  = new Vector3(0.5f, 0.5f, 0.5f);
	...
	//	public void updateVisualNetworkHandler(reflectionPath directSound,List<reflectionPath> reflections) {
	//		reflectionPath copyDS = directSound;
	//		List<reflectionPath> copyRFS = reflections;
	//		StartCoroutine(updateVisualNetwork(copyDS,copyRFS));
	//	}
